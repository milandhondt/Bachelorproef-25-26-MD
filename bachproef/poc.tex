...
Uit de analyse van het probleemdomein (Hoofdstuk \ref{ch:probleemdomein}) is duidelijk gebleken dat lectoren voornamelijk botsen op tijdgebrek, inconsistenties bij handmatig nakijken, en de cognitieve belasting van het doorploegen van grote hoeveelheden code. Om deze specifieke pijnpunten aan te pakken, wordt in dit hoofdstuk de praktische implementatie van de AI-evaluator besproken in de vorm van een \textit{Proof-of-Concept} (PoC). De opgebouwde testomgeving en het verloop van de geautomatiseerde evaluatie worden in detail toegelicht, waarna de technologische keuzes en een concreet voorbeeld van de input en output worden gepresenteerd. Tot slot worden de belangrijkste uitdagingen en beperkingen van deze implementatie besproken.

\section{De Evaluatieflow}

De geautomatiseerde evaluatie van studentenprojecten binnen de PoC wordt aangestuurd door een gecentraliseerd script (\texttt{run-evaluation.js}). Dit proces is opgebouwd uit verschillende opeenvolgende fasen om een veilige, contextbewuste en doeltreffende code-evaluatie te garanderen.

\vspace{1em}

\subsection{Stap 1: Configuratie en Sandbox Setup}

Voordat de evaluatie start, wordt het systeem geconfigureerd met de nodige parameters. Via command-line argumenten wordt gespecificeerd voor welk opleidingsonderdeel (Front-End Web Development of Web Services) de evaluatie wordt uitgevoerd. Dit is essentieel omdat de opdrachten en de bijbehorende evaluatiecriteria tussen beide vakken verschillen. Op basis van deze input selecteert het script automatisch de relevante set criteria (markdown-bestanden) en het bijbehorende voorbeeldproject dat als referentiepunt dient.

\vspace{1em}

De veiligheid en integriteit van het evaluatieproces vormen een primaire zorg bij het geautomatiseerd uitvoeren en inspecteren van studentencode. Daarom start de flow met het opzetten van een geïsoleerde 'sandbox' omgeving via E2B. Deze sandbox fungeert als een afgeschermde virtuele machine met een vaste time-out limiet. Zodra de sandbox operationeel is, wordt zowel de broncode van de student als het geselecteerde voorbeeldproject ingeladen. Dit stelt de later aangeroepen AI-agents in staat om de implementatie van de student gericht te vergelijken met het beoogde resultaat van de specifieke opdracht.
...
Naast de beslisstatus en een rubric-indicatie (waarbij de agent het project vergelijkt met een referentieproject), bevat het resulterende JSON-object ook een gedetailleerde argumentatie (met verwijzingen naar specifieke codebestanden of logica) en constructieve feedback.
...
De AI-agent voert op basis hiervan zijn verkenning uit in de sandbox en genereert een gestructureerde JSON-respons. Codefragment~\ref{lst:criterium-json} toont de werkelijke output van de agent na evaluatie, inclusief de verplichte rubric-indicatie, de uitgebreide beoordeling, de onderbouwing (bewijs), feedback voor de student en interne metadata over de gebruikte agent en routering. Het gehele evaluatierapport bundelt al deze individuele criteria én, in het geval van de ontvankelijkheidsflow, een algemeen ontvankelijkheidsverdikt.

\begin{listing}[htbp]
\begin{minted}{json}
{
  "criteria": "Componentstructuur van de React-frontend",
  "aanwezig": "aanwezig",
  "rubricIndicatie": "analoog",
  "bewijs": "Het project heeft een duidelijke componentstructuur met een components/ map (/home/user/fews-project-01/CollectionTracker/src/components) die meerdere herbruikbare componenten bevat. Er zijn submappen voor specifieke functionaliteiten en een pages/ map voor pagina-niveau componenten zoals Layout.jsx en Homepage.jsx. De structuur volgt een logische scheiding en is vergelijkbaar met de structuur in het referentieproject.",
  "feedback": "De componentstructuur is goed georganiseerd met een duidelijke scheiding tussen herbruikbare componenten en pagina's. Er zijn meerdere componentbestanden geïdentificeerd die hergebruik bevorderen. Overweeg om eventueel nog meer specifieke submappen aan te maken voor betere organisatie wanneer het project verder groeit.",
  "_meta": {
    "model": "qwen3-coder:480b-cloud",
    "sandboxId": "i4cjh6ssd4q7hr057r4z1",
    "project": "fews-project-01",
    "domain": "frontend",
    "criteriaFile": "fe-componenten-meerdere.md",
...
\end{minted}
\caption{Gegenereerde JSON-output van de specialist-agent, inclusief rubricIndicatie.}
...
\subsection{Uitvoeringstijd en Reasoning (Thinking)}
Een belangrijke factor om rekening mee te houden is de doorlooptijd van een evaluatieronde. Omdat het gebruikte LLM (qwen3-coder:480b-cloud) gebruikmaakt van een uitgebreid \textit{thinking}- of redeneerproces, ligt de kwaliteit en de betrouwbaarheid van de uiteindelijke beoordeling erg hoog. Dit diepgaande redeneerproces (waarbij het model 'denkt' in verborgen tokens voordat de effectieve JSON gegenereerd wordt) vergt echter initieel extra rekentijd. In de beginfase van het \textit{proof-of-concept}, in combinatie met het iteratief aanroepen van verscheidene tools, leidde dit soms tot bijzonder lange wachttijden. 

\vspace{1em}

Dankzij verdere optimalisaties zijn deze tijden intussen sterk gereduceerd en gestabiliseerd. Tijdens de huidige iteratie van de validatie nam een ontvankelijkheidsrun per project nog ongeveer 8 minuten in beslag, terwijl een uitgebreidere evaluatiecriteria-run rond de 15 minuten duurde.