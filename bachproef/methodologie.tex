...
\begin{itemize}
    ...
    \item prompt-engineering en fine-tuning technieken
\end{itemize}

Uit deze literatuurstudie werden verschillende relevante inzichten rondom het inzetten van LLM's verworven die voor de technische uitwerking enorm van pas zullen komen. Verder werd er als concrete deliverable een longlist opgesteld met poteniële LLM's die geschikt zijn voor code-evaluatie, en gebruikt zullen worden bij de technische uitwerking. Naast geschiktheid werden ook technische beperkingen zoals contextlimieten en tokenrestricties onderzocht.

\section{Fase 3: Ontwerp en opzetten testomgeving en PoC}

In de derde fase werd een reproduceerbare testomgeving ontwikkeld waarin verschillende LLM’s systematisch geëvalueerd konden worden. Dit omvat:
\begin{itemize}
...
\section{Fase 4: Kwantitatieve evaluatie van LLMs}

In deze fase werden geselecteerde LLM’s geëvalueerd op basis van:

\begin{itemize}
    \item correctheid van criteriadetectie
    \item consistentie met evaluaties van lectoren
    \item interne consistentie bij herhaalde evaluaties
    \item performantie (tijd en resourcegebruik)
    \item potentiële tijdswinst voor lectoren
\end{itemize}

De modellen werden eerst individueel beoordeeld en vervolgens onderling vergeleken.
...
Deze synthese leidde tot concrete aanbevelingen voor lectoren en hoe zij LLM's kunnen inzetten ter ondersteuning bij de evaluatie van studentenprojecten.
