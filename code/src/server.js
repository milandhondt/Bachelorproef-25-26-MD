...
async function startWizard() {
  ...
  try {
    ...
    const domainsToEvaluate = isFews ? ["frontend", "webservices"] : ["webservices"];
    ...
    if (tasks.length === 0) {
      console.error(`No Markdown criteria files found for the targeted domains.`);
      process.exit(1);
    }
    ...
    for (const result of results) {
      ...
      console.log(`   Aanwezig:  ${result.output.aanwezig}`);
